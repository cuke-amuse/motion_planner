...
//const unordered_map<std::string, float> DecisionAgent2::actionReward = {{"normal",3}, {"acc",0}, {"dec",-1}, {"stop",-1}, {"left", 5},{"right", 4}};
...
{    
    ...
    while(abs(carpos.x - path[path.size()-1].x) > 5) {
        ...
        mycar->update();
        for (unsigned int i = 0; i < model.getOtherCars().size(); ++i) {
...
{    
    ...
    for (unsigned int i = 0; i < paths.size(); i++) {
        ...
        std::cout << "Decision evaluate score:" << score << "/" <<  paths.size() << std::endl;
    }
    mypath = paths[index];
...
bool DecisionAgent2::isChangeRequired(Car* mycar, const Model& model) {
    ...
    if (std::abs(host->getPos().y - goal.y) < 5)
        return false;
    return true;
}
#endif
