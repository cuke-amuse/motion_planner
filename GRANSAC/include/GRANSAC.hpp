...
// threshlod + iteration cnt + Model select + believe degree
namespace GRANSAC {
	// T - AbstractModel
	...
	public:
		...
		std::shared_ptr<T> GetBestModel(void) const
		{ return m_BestModel; };
		const std::vector<std::shared_ptr<AbstractParameter>>& GetBestInliers(void) const
