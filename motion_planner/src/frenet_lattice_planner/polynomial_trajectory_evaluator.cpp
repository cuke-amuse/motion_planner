...
namespace planning
{
        ...
        ref_line_(ref_line)
  {
    ...
    {
      ROS_INFO("Trajectory Evaluate %lu", lon_trajectory_vec.size());
                                                 ...
                                                 const std::shared_ptr<common::Polynomial> &lat_traj)
  {
    ...
    double res = lon_collision_cost * PlanningConfig::Instance().lattice_weight_collision() +
    ...
    std::cout << " lon_target_cost: " << lon_target_cost << ",lon_jerk_cost: " << lon_jerk_cost
              ...
              << ", centripental_cost: " << centripental_cost << " total:" << res << std::endl;
#endif
    return res;
  }
...
// lateral evaluate , should be add while offset the reference line
                                                      ...
                                                      const std::shared_ptr<common::Polynomial> &lon_trajectory)
  {
    ...
    std::vector<double> s_values(evaluation_horizon, 0);
                                                      ...
                                                      const PlanningTarget &planning_target)
  {
    ...
    //ROS_INFO("LonTargetCost: the desired vel is %lf", planning_target.desired_vel);
    ...
    std::cout << "desired vel:" << planning_target.desired_vel << " speed cost: " << speed_cost << "dist_travelled_cost: " << dist_travelled_cost << std::endl;
