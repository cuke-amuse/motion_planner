...
namespace planning {
  // fix the thread to compare obstacle 15 thread
MotionPlanner::MotionPlanner(const ros::NodeHandle &nh) : nh_(nh), thread_pool_size_(15) 
{
  ...
  //uint16_t port = 0;
  //ct_ = carla::client::Client(host, port);
  //ct_.SetTimeout(10s);

        // std::cout << "confirm the ct vesion:" << ct_.GetClientVersion()   << std::endl;  // check whether it works well
      map_ = ct_.GetWorld().GetMap();
      if (map_ == nullptr) {
        return;
      }
      std::cout << "confirm the ct vesion:" << ct_.GetClientVersion()  << " map is " << map_->GetName() << std::endl;  // check whether it works well
}
...
//Output Global Router infor
void MotionPlanner::InitRouteRecycle()
{
    if (inited_ == false) {
      inited_ = true;
      std::mt19937_64 rng((std::random_device())());

      std::cout << "init route confirm the ct vesion:" << ct_.GetClientVersion()  << "map is " << map_->GetName() << std::endl;   // check whether it works well

      carla::nav::Navigation na;
      carla::geom::Location start = carla::geom::Location(ego_object_.pose.position.x, ego_object_.pose.position.y, ego_object_.pose.position.z);
      carla::geom::Transform tf = RandomChoice(map_->GetRecommendedSpawnPoints(), rng);
      carla::geom::Location end = tf.location;
      std::cout << "recommand :" << start.x  << "/" << start.y << "to:"<< end.x << "/" << end.y << std::endl; 
      std::vector<unsigned char>  area;  

      if (na.GetPath(start, end, nullptr, routerPath_, area)) {
          std::cout << "Get c++ path" << std::endl;
      }
    }
}
...
void MotionPlanner::InitSubscriber() {
  ...
  this->traffic_lights_subscriber_ = nh_.subscribe<carla_msgs::CarlaTrafficLightStatusList>(
      ...
      [this](const carla_msgs::CarlaTrafficLightStatusList::ConstPtr &traffic_light_status_list) {
        ...
        for (const auto &traffic_light_status : traffic_light_status_list->traffic_lights) {
          ...
          ROS_INFO("Get traffic light status");
        }
      });
  this->traffic_lights_info_subscriber_ = nh_.subscribe<carla_msgs::CarlaTrafficLightInfoList>(
      ...
      [this](const carla_msgs::CarlaTrafficLightInfoList::ConstPtr &traffic_lights_info_list) {
        ...
        ROS_INFO("Get traffic light info");
      });
...
// receive the route result from cmpf_route_planner
  this->goal_route_subscriber_ = nh_.subscribe<cmpf_msgs::Route>(
      common::topic::kRoute, 1,
      [this](const cmpf_msgs::Route::ConstPtr &goal_route) {
        std::cout << "goal route subscriber : " << ego_vehicle_id_ << std::endl;
        if (ego_vehicle_id_ == -1) {
          return;
        }
        if (!ReRouteCMPF(goal_route)) {
         return;
        }
      });
}
...
//  optimal_trajectory_marker.color.g =0.7;
  ...
  for (const auto &tp : optimal_trajectory.trajectory_points) {
    ...
    p.y = tp.path_point.y;
    p.z = 2;
    optimal_trajectory_marker.points.push_back(std::move(p));
  }
...
bool  MotionPlanner::GetEgoVehicleRoutes(geometry_msgs::Pose &start_pose , geometry_msgs::Pose &destination) {
  ...
  if (!get_ego_vehicle_route_client_.call(srv)) {
    std::cout <<  "vehicle get service err" << std::endl;
    return -1;
  }
  if (srv.response.route.way_points.size() < 20) {
    std::cout <<  "vehicle route po err" << std::endl;
    return -2;
  }
  return reference_generator_->UpdateRouteResponse(srv.response);
}

bool  MotionPlanner::GetEgoVehicleRouteCMPF(const cmpf_msgs::Route::ConstPtr &path) 
{
  if (path->waypoints.size() < 5) {
    std::cout <<  "vehicle sub po err" << std::endl;
    return -2;
  }
 reference_generator_->Init();   
  return reference_generator_->UpdateRouteCMPF(path);
}
