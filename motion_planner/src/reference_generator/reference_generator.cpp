...
// transform from WayPoint class to msg
bool ReferenceGenerator::UpdateRouteCMPF(const cmpf_msgs::Route::ConstPtr &path)
{
    std::lock_guard<std::mutex> lock_guard(route_mutex_);
    planning_msgs::WayPoint wp;
  for (const auto& ele : path->waypoints) {
    wp.pose.position.x = ele.x;
    wp.pose.position.y = ele.y;
    wp.lane_width = ele.laneWidth;  // assume
    route_info_.main_lane.push_back(std::move(wp));
  }
  has_route_ = true;
  return true;
}
...
// transform from WayPoint class to msg
bool ReferenceGenerator::UpdateRouteSub(std::vector<carla::SharedPtr<carla::client::Waypoint>> &wpVec) //const nav_msgs::Path::ConstPtr &waypoints) 
{
    std::lock_guard<std::mutex> lock_guard(route_mutex_);
    //auto raw_ref_lane = route_response.route;
    //route_info_.main_lane = waypoints->poses;
    std::vector<planning_msgs::WayPoint> tempWay;
    planning_msgs::WayPoint temp;
    //for (const auto& ele : waypoints->poses) {
      for (const auto& ele : wpVec) {
        temp.id = ele->GetId();
        temp.road_id = ele->GetRoadId();
        temp.section_id = ele->GetSectionId();
        temp.lane_id = ele->GetLaneId();
        temp.is_junction = ele->IsJunction();
        temp.has_left_lane = (ele->GetLeft() == nullptr) ? false : true;
        temp.has_right_lane = (ele->GetRight() == nullptr) ? false : true;
        //temp.has_value
        temp.lane_width = ele->GetLaneWidth();
        
        if (temp.has_right_lane) {
            temp.right_lane_width = ele->GetRight() ->GetLaneWidth();
        }
        if (temp.has_left_lane) {
            temp.left_lane_width = ele->GetLeft()->GetLaneWidth();
        }
        //temp.lane_change = 
        //temp.lane_type = static_cast<planning_msgs::LaneType>(ele->GetType());
        //temp.left_lane_marking
        //temp.right_lane_marking
        //temp.junction = ele->GetJunction();
        //temp.road_option
        //temp.s
        temp.pose.position.x = ele->GetTransform().location.x;
        temp.pose.position.y = ele->GetTransform().location.y;
        temp.pose.position.z = ele->GetTransform().location.z;
         //ROS_INFO("transform to (%lf/%lf)",  temp.pose.position.x,  temp.pose.position.y);
        // waypoint 为什么还带有位资,??
       // temp.pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(ele->GetTransform().rotation.roll,
         //  ele->GetTransform().rotation.pitch, ele->GetTransform().rotation.yaw);
        //temp.pose.orientation.x = ele->GetTransform().rotation.pitch;
        //temp.pose.orientation.y = ele->GetTransform().rotation.yaw;
        //temp.pose.orientation.z = ele->GetTransform().rotation.roll;
        //temp.pose.orientation.w = ele.pose.orientation.w;
        tempWay.push_back(temp);  
        std::cout << "temp.road_id" << temp.road_id << std::endl;
      }
  std::cout << "transfer to rec " << tempWay.size() << std::endl;
  route_info_.main_lane = tempWay;
  has_route_ = true;
  return true;
}
...
void ReferenceGenerator::GenerateThread() {
  while (!is_stop_) {
    static constexpr int32_t kSleepTime = 2000;  // milliseconds
    std::this_thread::sleep_for(std::chrono::milliseconds(kSleepTime));
    auto start_time = std::chrono::system_clock::now();
    if (!has_route_) {
      ROS_FATAL("Routing is not ready.");  // global path
      continue;
    }
